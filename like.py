...
def test_user_rated_less_than_10_movies(self, mocker):
    # Arrange
    handle_dismiss = mocker.Mock()
    handle_click = mocker.Mock()
    dismissible_ref = mocker.Mock()
    navigationBar = mocker.Mock()
    page = mocker.Mock()
    user = mocker.Mock()
    user.movie_rating_data_frame.__len__.return_value = 5

    # Act
    result = return_like_page(handle_dismiss, handle_click, dismissible_ref, navigationBar, user, page)

    # Assert
    assert result == base_like_page(handle_dismiss, handle_click, dismissible_ref, navigationBar, user, page)

# Handles the case where the user has seen all available shows
def test_handles_all_shows_seen(self, mocker):
    # Mocking dependencies
    mock_page = mocker.Mock()
    mock_user = mocker.Mock()
    mock_user.seen_show = {'s1', 's2', 's3'}
    mock_user.profile = "test_profile"
    mock_user.movie_list_data_frame = mocker.Mock()
    mock_user.movie_list_data_frame['show_id'].values = ['s1', 's2', 's3']

    # Mocking the return values for get_popularity and get_show
    mocker.patch('machine_learning_utils.get_popularity', return_value={'s1': 10, 's2': 8, 's3': 6})
    mocker.patch('machine_learning_utils.get_show', return_value=None)

    # Call the function under test
    view = base_like_page(mocker.Mock(), mocker.Mock(), mocker.Mock(), mocker.Mock(), mock_user, mock_page)

    # Assertions
    assert isinstance(view, ft.View)
    assert any(control.value == "Please, rate the next 10 movies:" for control in view.controls[0].content.controls)

# Returns a view with the correct movie details when a valid user and page are provided
def test_returns_correct_movie_details(self, mocker):
    # Mocking dependencies
    mock_page = mocker.Mock()
    mock_user = mocker.Mock()
    mock_user.seen_show = set()
    mock_user.profile = "test_profile"
    mock_user.movie_list_data_frame = mocker.Mock()
    mock_user.movie_list_data_frame[user.movie_list_data_frame['show_id'] == 's1'].iloc[0] = {
        "show_id": "s1",
        "title": "Test Movie",
        "release_year": "2023"
    }
    mocker.patch('machine_learning_utils.get_popularity', return_value={'s1': 10})
    mocker.patch('machine_learning_utils.get_show', return_value='s1')
    ...
    # Assertions
    assert isinstance(view, ft.View)
    assert any(control.value == "Test Movie" for control in view.controls[0].content.controls[1].content.controls)